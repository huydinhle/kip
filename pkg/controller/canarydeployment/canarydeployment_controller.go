...
// Add creates a new CanaryDeployment Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileCanaryDeployment{client: mgr.GetClient(), scheme: mgr.GetScheme()}
}

// add adds a new Controller to mgr with r as the reconcile.Reconciler
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	// Create a new controller
	c, err := controller.New("canarydeployment-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource CanaryDeployment
	err = c.Watch(&source.Kind{Type: &appv1alpha1.CanaryDeployment{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileCanaryDeployment{}

// ReconcileCanaryDeployment reconciles a CanaryDeployment object
type ReconcileCanaryDeployment struct {
...
// Reconcile reads that state of the cluster for a CanaryDeployment object and makes changes based on the state read
// and what is in the CanaryDeployment.Spec
// TODO(user): Modify this Reconcile function to implement your Controller logic.  This example creates
// a Deployment as an example
// Note:
// The Controller will requeue the Request to be processed again if the returned error is non-nil or
// Result.Requeue is true, otherwise upon completion it will remove the work from the queue.
func (r *ReconcileCanaryDeployment) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	reqLogger := log.WithValues("Request.Namespace", request.Namespace, "Request.Name", request.Name)
	reqLogger.Info("Reconciling CanaryDeployment")

	// Fetch the CanaryDeployment instance
	instance := &appv1alpha1.CanaryDeployment{}
...
func newDeploymentForCR(cr *appv1alpha1.CanaryDeployment) *appsv1.Deployment {
